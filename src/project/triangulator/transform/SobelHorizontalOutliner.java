...
import project.triangulator.filter.SobelHorizontalFilter;
...
public class SobelHorizontalOutliner {
	...
	public static OutlinedImage outline( Image base ) {
		...
		for ( int y = 0; y < base.getHeight(); y++ ) {
			...
			for ( int x = 0; x < base.getWidth(); x++ ) {
				...
				/**
				 * The "Sobel-Operator" determines a value
				 * defining the significance of an edge.
				 * This value must exceed the threshold so that
				 * the regarding pixel can be considered a
				 * proper vertex.
				 */
				if ( ColorDecomposer.getRed( SobelHorizontalFilter.filter( source , x , y ) ) >= Application.OUTLINE_THRESHOLD ) {
