...
import project.triangulator.filter.SobelVerticalFilter;
...
public class SobelVerticalOutliner {
	...
	public static OutlinedImage outline( Image base ) {
		...
		for ( int y = 0; y < base.getHeight(); y++ ) {
			...
			for ( int x = 0; x < base.getWidth(); x++ ) {
				 ...
				 * proper vertex.
				 */
				if ( ColorDecomposer.getRed( SobelVerticalFilter.filter( source , x , y ) ) >= Application.OUTLINE_THRESHOLD ) {
